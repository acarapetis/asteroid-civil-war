...
bool R2::operator==(R2 other) const {
...
double R2::lengthSquared() const {
    return x*x + y*y;
}
double R2::length() const {
    return sqrt(this->lengthSquared());
}
double R2::angle() const {
    return atan2(y,x);
}
...
R2 R2::rotateAndScale(double radians, R2 pivot, double scale) const {
    ...
    return R2(l*cos(r.angle() + radians), l*sin(r.angle() + radians)) + pivot;
}
...
R2 R2::coerceDelta(R2 min, R2 max) {
    R2 delta = R2(0,0);
    if (x < min.x) { delta.x = min.x - x; x = min.x; }
    if (x > max.x) { delta.x = max.x - x; x = max.x; }
    if (y < min.y) { delta.y = min.y - y; y = min.y; }
    if (y > max.y) { delta.y = max.y - y; y = max.y; }
    return delta;
}
...
bool pointInPolygon(R2 point, std::list<R2> polygon) {
    int c = 0;
    R2 prev;
    for (list<R2>::const_iterator i = polygon.begin(); i != polygon.end(); i++) {
        R2 start, end = *i;
        if (i == polygon.begin())
        ...
        if (intersectsPositiveX(start - point, end - point)) ++c;
        prev = end;
    }
