...
void Sprite::draw(R2 pivot, double radians, double scale) {
    ...
    R2 ic = R2(al_get_bitmap_width (bitmap)/2,
                      al_get_bitmap_height(bitmap)/2);
    drawTransformedBitmap(bitmap, ic.x, ic.y, nc.x, nc.y, 
            this->scale * scale, this->scale * scale,
            radians + rotation);
#ifdef DEBUG
    printf("SPRITE center=%i,%i scale=%f, rotation=%f\n",
            int(nc.x), int(nc.y), this->scale*scale, radians+rotation);
#endif
}
string Sprite::describe() {
    return "Sprite";
}
...
// Circle
Circle::Circle(R2 center, ALLEGRO_COLOR color, double radius, bool filled, double border, ALLEGRO_COLOR borderColor) : center(center), color(color), radius(radius), filled(filled), border(border), borderColor(borderColor) { 
}
Circle::Circle(R2 center, ALLEGRO_COLOR color, double radius, bool filled, double border) : center(center), color(color), radius(radius), filled(filled), border(border) {
...
// Line
Line::Line(R2 start, R2 end, ALLEGRO_COLOR color, double thickness) : start(start), end(end), color(color), thickness(thickness) {
}
void Line::draw(R2 pivot, double radians, double scale) {
    ...
    //cout   << tStart.x << "," << tStart.y << " " 
                //<< tEnd.x   << "," << tEnd.y   << endl;
    drawTransformedLine(tStart.x, tStart.y, tEnd.x, tEnd.y, color, int(thickness * scale));
    /*
    if (thickness * scale > 2) {
        al_draw_filled_circle(tStart.x, tStart.y, int(thickness * scale / 2), color);
        al_draw_filled_circle(  tEnd.x,   tEnd.y, int(thickness * scale / 2), color);
    }
    */
}
...
DynamicDrawable::DynamicDrawable( boost::function<shared_ptr<Drawable> ()> generator ) :
    generator(generator)
{
}
shared_ptr<Drawable> DynamicDrawable::freeze() 
{
    return generator();
}
...
string Polygon::describe() {
    string s = "Polygon: ";
    for (std::list<R2>::const_iterator i = points.begin(); i != points.end(); i++) {
        if (i != points.begin())
            s += " -> ";
...
void Polygon::draw(R2 pivot, double radians, double scale) {
    ...
    for (std::list<R2>::const_iterator i = points.begin(); i != points.end(); i++) {
        ...
        drawTransformedLine(
                tstart.x, tstart.y, tend.x, tend.y, color, int(thickness * scale)
        );
        prev = end;
    }
...
bool Polygon::mouseInside(R2 pivot, double radians, double scale) {
    std::list<R2> ps = ltransform(translate(points, pivot), radians, pivot, scale);
    return pointInPolygon(game.camera.s2w(game.mouse->position), ps);
}
std::list<R2> Polygon::getPoints() {
    return points;
}
    ...
    for (list<R2>::iterator i = tpoints.begin(); i != tpoints.end(); i++) {
        ...
        if (intersectsPositiveX(*start, *end)) {
            if (!slicing) {
                ...
                roid = translate(
                        ltransform(roid, angle, origin, 1),
                origin);
...
CompoundDrawable::CompoundDrawable(vector<shared_ptr<Drawable> > visualComponents) :
...
Instance::Instance(shared_ptr<Drawable> visual, R2 center, double rotation, double scale) : 
...
Gravitee::Gravitee(R2 center, R2 velocity, double mass, bool obeyGravity) : center(center), velocity(velocity), obeyGravity(obeyGravity), mass(mass)
{
}
...
//A physical majigger
GravInstance::GravInstance(shared_ptr<Drawable> visual, R2 center, R2 velocity, double scale, bool obeyGravity) :
    Gravitee(center, velocity, 1, obeyGravity), scale(scale), visual(visual) {
}
void GravInstance::tick(double dt) {
    center += velocity * dt;
}
void GravInstance::draw() {
    this->visual->draw(center, 0, this->scale);
}

Ship::Ship(shared_ptr<Drawable> visual, shared_ptr<EmitterType> et, R2 center, R2 velocity, double scale, bool obeyGravity, double rotation) : 
    GravInstance(visual, center, velocity, scale, obeyGravity), rotation(rotation),
    emitter( shared_ptr<EmitterInstance>( 
