...
bool update(double dt) {
    ...
    for (astV::const_iterator i = asteroids.begin(); 
            i != asteroids.end(); i++) {
...
bool draw(double dt) {
    ...
    for (double ax = 0; ax < game.screenSize.x; ax += 20) {
        for (double ay = 0; ay < game.screenSize.y; ay += 20) {
            ...
            drawTransformedFilledCircle(p.x, p.y, game.camera.s2w(4), al_map_rgba_f(0.0,0.0,1.0, ddt(p)));
        }
    }
    */
    ...
    al_draw_text(game.smallfont, 0, 30, -1, (astCountStr + " asteroids").c_str());
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 10; i++) {
        shared_ptr<Asteroid> a(new Asteroid(randFactor()*80 + 20, R2(500,300).randomise(), 0, 1, randFactor()*PI - PI/2));
    ...
    //smoke = shared_ptr<Sprite>(new Sprite(R2(), al_load_png("bigsmoke.png"), 0, 0.05));
    ...
    ship = shared_ptr<Ship>(new Ship(sp, shared_ptr<EmitterType>(new EmitterType(gsmoke, 10))));
