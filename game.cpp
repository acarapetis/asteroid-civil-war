...
ALLEGRO_DISPLAY* initDisplay() {
    ...
    ALLEGRO_DISPLAY *display = al_create_display(game.screenSize.x, game.screenSize.y);
    if (!display)
        throw Error::Display;
...
ALLEGRO_EVENT_QUEUE* initQueue() {
    ...
    al_register_event_source(q, (ALLEGRO_EVENT_SOURCE*)al_get_keyboard_event_source());
    al_register_event_source(q, (ALLEGRO_EVENT_SOURCE*)al_get_mouse_event_source());
...
Game::Game() :
    screenSize ( R2(800,480)        ),
    resizeable ( true               ),
    camera     ( Camera(R2(), 0, 1) )
{
}
...
R2 Game::screenCenter() {
    return this->screenSize/2;
}

void Game::run(boost::function<bool(double)> update, boost::function<bool(double)> draw) {
